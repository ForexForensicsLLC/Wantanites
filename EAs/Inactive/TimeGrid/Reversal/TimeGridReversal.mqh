...
void TimeGrid::PlaceOrders()
{
    ...
    double lotSize = mLotSize * (mPreviousSetupTickets.Size() + 1);

    EAHelper::PlaceMarketOrder<TimeGrid>(this, entry, stopLoss, lotSize);
...
void TimeGrid::ManagePreviousSetupTicket(int ticketIndex)
{
    ...
    if (EAHelper::CloseTicketIfPastTime<TimeGrid>(this, mPreviousSetupTickets[ticketIndex], 23, 59))
    {
        return;
    }

    double equityPercentChange = EAHelper::GetTotalPreviousSetupTicketsEquityPercentChange<TimeGrid>(this, mStartingEquity);
    if (equityPercentChange <= -3)
    {
