...
bool NeighborMBs::Confirmation()
{
    ...
    bool deepEnoughIntoZone = EAHelper::PriceIsFurtherThanPercentIntoHoldingZone<NeighborMBs>(this, mSetupMBT, mFirstMBInSetupNumber, furthestPrice, 0.8);
    return hasTicket || deepEnoughIntoZone;
}
...
void NeighborMBs::PlaceOrders()
{
    ...
    double entry = tempZoneState.PercentOfZonePrice(0.1);
    double stopLoss = tempZoneState.ExitPrice();
...
void NeighborMBs::ManageCurrentPendingSetupTicket()
{
    ...
    if (tempZoneState.Number() != mEntryZone)
    {
        InvalidateSetup(true);
        return;
    }
