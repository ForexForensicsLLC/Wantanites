...
bool NeighborMBs::Confirmation()
{
    ...
    if (tempMBState.EndIndex() - pendingMBStart > 1)
    {
        return false;
    }
    ...
    bool sameTypeCandleInZone = false;
    bool inZone = EAHelper::CandleIsInZone<NeighborMBs>(this, mSetupMBT, mFirstMBInSetupNumber, 1, false);

    if (mSetupType == OP_BUY)
    {
        if (!MQLHelper::GetLowestIndexBetween(mEntrySymbol, mEntryTimeFrame, tempMBState.EndIndex() - 1, 1, true, furthestIndex))
        {
            return false;
        }

        furthestPrice = iLow(mEntrySymbol, mEntryTimeFrame, furthestIndex);
        sameTypeCandleInZone = CandleStickHelper::IsBullish(mEntrySymbol, mEntryTimeFrame, 1);
    }
    else if (mSetupType == OP_SELL)
    {
        if (!MQLHelper::GetHighestIndexBetween(mEntrySymbol, mEntryTimeFrame, tempMBState.EndIndex() - 1, 1, true, furthestIndex))
        {
            return false;
        }

        furthestPrice = iHigh(mEntrySymbol, mEntryTimeFrame, furthestIndex);
        sameTypeCandleInZone = CandleStickHelper::IsBearish(mEntrySymbol, mEntryTimeFrame, 1);
    }
    ...
    return hasTicket || (inZone && sameTypeCandleInZone);
}
