    ...
    if (EAHelper::CheckSetSingleMBSetup<NeighborMBs>(this, mSetupMBT, mFirstMBInSetupNumber, mSetupType))
    {
        ...
        if (mSetupType == OP_BUY)
        {
            for (int i = 1; i <= 5; i++)
            {
                ...
                if (!mSetupMBT.GetNthMostRecentMB(i, previousMBState))
                {
                    return;
                }

                if (iHigh(mEntrySymbol, mEntryTimeFrame, previousMBState.HighIndex()) > iHigh(mEntrySymbol, mEntryTimeFrame, tempMBState.HighIndex()))
                {
                    return;
                }
            }
        ...
        else if (mSetupType == OP_SELL)
        {
            for (int i = 1; i <= 5; i++)
            {
                ...
                if (iLow(mEntrySymbol, mEntryTimeFrame, previousMBState.LowIndex()) < iLow(mEntrySymbol, mEntryTimeFrame, tempMBState.LowIndex()))
                {
                    return;
                }
            }
...
bool NeighborMBs::Confirmation()
{
    ...
    if (height / tempMBState.Height() > 1.5 || height / tempMBState.Height() < 0.5)
    {
        return false;
    }
    ...
    int start = MathMin(pendingMBStart, tempZoneState.StartIndex() - tempZoneState.EntryOffset() - 2);
    bool twoConsecutiveCandlesInOurFavor = false;
    int candleCount = 0;
    ...
    if (mSetupType == OP_BUY)
    {
        ...
        {
            if (iLow(mEntrySymbol, mEntryTimeFrame, i + 1) <= tempZoneState.EntryPrice())
            {
                if (CandleStickHelper::LowestBodyPart(mEntrySymbol, mEntryTimeFrame, i) > CandleStickHelper::LowestBodyPart(mEntrySymbol, mEntryTimeFrame, i + 1))
                {
                    candleCount += 1;
                }
                else
            ...
            if (candleCount >= 2)
            {
                twoConsecutiveCandlesInOurFavor = true;
                break;
            }
        }
    }
    else if (mSetupType == OP_SELL)
    {
        for (int i = start; i >= 1; i--)
        {
            if (iHigh(mEntrySymbol, mEntryTimeFrame, i + 1) >= tempZoneState.EntryPrice())
            {
                if (CandleStickHelper::HighestBodyPart(mEntrySymbol, mEntryTimeFrame, i) < CandleStickHelper::HighestBodyPart(mEntrySymbol, mEntryTimeFrame, i + 1))
                {
                    candleCount += 1;
                }
                else
    ...
    return twoConsecutiveCandlesInOurFavor;
}
