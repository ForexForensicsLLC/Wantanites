...
bool MBInnerBreak::Confirmation()
{
    ...
    if (mSetupType == OP_BUY)
    {
        ...
        for (int i = mostRecentPushUp - 1; i >= 1; i--)
        {
            ...
            {
                // don't enter if the break happened more than 10 candles prior
                if (i > 7)
                {
                    return hasTicket;
                }
        ...
        if (!brokePushUp || breakCandleIndex < 3)
        {
            return false;
        }
        ...
        {
            if (CandleStickHelper::IsBearish(mEntrySymbol, mEntryTimeFrame, i) && iClose(mEntrySymbol, mEntryTimeFrame, i) < iLow(mEntrySymbol, mEntryTimeFrame, i + 1))
            {
                retracement = true;
                break;
            }
        ...
        // Middle Candle Entry
        bool middleIsHighest = iHigh(mEntrySymbol, mEntryTimeFrame, 2) > iHigh(mEntrySymbol, mEntryTimeFrame, 3) &&
                               iHigh(mEntrySymbol, mEntryTimeFrame, 2) > iHigh(mEntrySymbol, mEntryTimeFrame, 1);
        bool middleBodyWithinFirst = CandleStickHelper::HighestBodyPart(mEntrySymbol, mEntryTimeFrame, 2) < iHigh(mEntrySymbol, mEntryTimeFrame, 3) &&
                                     CandleStickHelper::LowestBodyPart(mEntrySymbol, mEntryTimeFrame, 2) > iLow(mEntrySymbol, mEntryTimeFrame, 3);
        bool thirdDidNotBreakSecond = iClose(mEntrySymbol, mEntryTimeFrame, 1) > iLow(mEntrySymbol, mEntryTimeFrame, 2);

        if (!middleIsHighest || !middleBodyWithinFirst || !thirdDidNotBreakSecond)
        {
            return hasTicket;
        }
    }
    else if (mSetupType == OP_SELL)
    {
        ...
        for (int i = breakCandleIndex - 1; i >= 1; i--)
        {
            if (CandleStickHelper::IsBullish(mEntrySymbol, mEntryTimeFrame, i) && iClose(mEntrySymbol, mEntryTimeFrame, i) > iHigh(mEntrySymbol, mEntryTimeFrame, i + 1))
            {
                retracement = true;
                break;
            }
        ...
        // Middle Candle Entry
        bool middleIsLowest = iLow(mEntrySymbol, mEntryTimeFrame, 2) < iLow(mEntrySymbol, mEntryTimeFrame, 3) &&
                              iLow(mEntrySymbol, mEntryTimeFrame, 2) < iLow(mEntrySymbol, mEntryTimeFrame, 1);
        bool middleBodyWithinFirst = CandleStickHelper::HighestBodyPart(mEntrySymbol, mEntryTimeFrame, 2) < iHigh(mEntrySymbol, mEntryTimeFrame, 3) &&
                                     CandleStickHelper::LowestBodyPart(mEntrySymbol, mEntryTimeFrame, 2) > iLow(mEntrySymbol, mEntryTimeFrame, 3);
        bool thirdDidNotBreakSecond = iClose(mEntrySymbol, mEntryTimeFrame, 1) < iHigh(mEntrySymbol, mEntryTimeFrame, 2);

        if (!middleIsLowest || !middleBodyWithinFirst || !thirdDidNotBreakSecond)
        {
            return hasTicket;
        }
    }
...
void MBInnerBreak::PlaceOrders()
{
    ...
    if (mSetupType == OP_BUY)
    {
        ...
        entry = iHigh(mEntrySymbol, mEntryTimeFrame, 2) + OrderHelper::PipsToRange(mMaxSpreadPips + mEntryPaddingPips);
    ...
    else if (mSetupType == OP_SELL)
    {
        ...
        entry = iLow(mEntrySymbol, mEntryTimeFrame, 2) - OrderHelper::PipsToRange(mEntryPaddingPips);
