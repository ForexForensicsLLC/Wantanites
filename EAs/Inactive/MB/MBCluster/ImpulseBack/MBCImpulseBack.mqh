...
void MBCluster::CheckSetSetup()
{
    ...
    if (EAHelper::CheckSetSingleMBSetup<MBCluster>(this, mSetupMBT, mFirstMBInSetupNumber, mSetupType))
    {
        ...
        if (tempMBState.Height() < OrderHelper::PipsToRange(500))
        {
            return;
        }

        // previous mb can't be too large
        // if (tempMBState.Height() > OrderHelper::PipsToRange(1250) || tempMBState.Width() > 60)
        // {
        //     return;
        // }
...
bool MBCluster::Confirmation()
{
    ...
    bool furthestInZone = EAHelper::CandleIsInZone<MBCluster>(this, mSetupMBT, mFirstMBInSetupNumber, 1, true);
    if (!furthestInZone)
    {
        return hasTicket;
    }

    bool impulseBackIntoZone = false;
    int pendingMBStart = EMPTY;
    ...
    if (mSetupType == OP_BUY)
    {
        ...
        // check the pending mb start seperately since it can be bullish at times but i don't want that to stop the rest of the setup
        if (CandleStickHelper::IsBearish(mEntrySymbol, mEntryTimeFrame, pendingMBStart) &&
            CandleStickHelper::PercentChange(mEntrySymbol, mEntryTimeFrame, pendingMBStart) <= -0.1)
        {
        ...
        {
            // need to have percent change in the move back into the zone
            if (CandleStickHelper::PercentChange(mEntrySymbol, mEntryTimeFrame, i) <= -0.1)
            {
                hasMinPercentChange = true;
            }
    ...
    else if (mSetupType == OP_SELL)
    {
        ...
        // check the pending mb start seperately since it can be bearish at times but i don't want that to stop the rest of the setup
        if (CandleStickHelper::IsBullish(mEntrySymbol, mEntryTimeFrame, pendingMBStart) &&
            CandleStickHelper::PercentChange(mEntrySymbol, mEntryTimeFrame, pendingMBStart) >= 0.1)
        {
            hasMinPercentChange = true;
        }
